...
```{r beer10-point, fig.cap="(ref:beer10-point-cap)"}
...
```{r beer10-point-lm, fig.cap="(ref:beer10-point-lm-cap)"}
...
lm.out<-lm(calories~alcohol, data=Beer) #this saves the linear model into a variable called lm.out
residuals(lm.out) #this finds the residuals for each x value based on the linear model, lm.out and displays them.
```
...
```{r Beer10-point-residuals, warning=FALSE, fig.cap="(ref:Beer10-point-residuals-cap)"}
...
```{r Beer10a-point-lm, warning=FALSE, fig.cap="(ref:Beer10a-point-lm)"}
